...
static bsec_library_return_t bme680_bsec_update_subscription(float sample_rate)
{
    ...
    requested_virtual_sensors[7].sensor_id = 2;
    requested_virtual_sensors[7].sample_rate = sample_rate;
    requested_virtual_sensors[8].sensor_id = 3;
    requested_virtual_sensors[8].sample_rate = sample_rate;
    requested_virtual_sensors[9].sensor_id = 4;
    requested_virtual_sensors[9].sample_rate = sample_rate;
    requested_virtual_sensors[10].sensor_id = 5;
    requested_virtual_sensors[10].sample_rate = sample_rate;
    requested_virtual_sensors[11].sensor_id = 10;
    requested_virtual_sensors[11].sample_rate = sample_rate;
    requested_virtual_sensors[12].sensor_id = 11;
    requested_virtual_sensors[12].sample_rate = sample_rate;
    requested_virtual_sensors[13].sensor_id = 0;
...
static void bme680_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_ready_fct output_ready)
{
    ...
    if (num_bsec_inputs > 0)
    {
        ...
        //index 3 is the tvoc ppm
        /* Iterate through the outputs and extract the relevant ones. */
        voc = bsec_outputs[3].signal;
        siaq = bsec_outputs[1].signal;
        siaq_accuracy = bsec_outputs[1].accuracy;

        
        for (index = 0; index < 13; index++)
        {   
            // printf("Index = %d, value = %f\n",index,bsec_outputs[index].signal );
        ...
        output_ready(timestamp, iaq, iaq_accuracy, temp, humidity, raw_pressure, raw_temp, 
            raw_humidity, raw_gas, bsec_status,voc,siaq, siaq_accuracy);
    }
}
